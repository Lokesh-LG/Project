...
function dataReload() {
  ...
  leftBarArr = [
    ...
    ["Show PGN", "Import PGN", "No More Changes"],
  ];
...
function makeTimer() {
  selectedStr =
    "<div class='col p-0 mb-2 mx-1 bg-transparent '><div class='row justify-content-center'><button type='button' class='p-3 btn btn-light w-100 h-100 btn-timer' onclick = confirmedTime()><span id = 'timeLimit'>Select Time</span></button></div></div>";
...
function makeLeftBar(actionsData) {
  leftStr =
    ...
    "<button class = 'p-3 btn btn-light btn-block-red w-100 h-100' onclick = 'undoMove()'>Undo Move</button>";
  leftStr += leftBarArrAll
    .map(function (ele, index) {
      return makeLeftDD(
        ele,
        index,
        leftBarOpenStatus[index],
        leftBarArr[index],
        actionsData && actionsData.dd === index ? actionsData : null
      );
    })
    .join("");
...
function makeLeftDD(txt, index1, open, options, actionsData) {
  let str =
    "<button class='p-3 btn btn-light btn-block h-100' onclick='showOptionsLeftDD(" +
    index1 +
    ",-1)' >" +
    txt +
    "<i class='float-end fas " +
    (open ? "fa-caret-up" : "fa-caret-down") +
    "'></i></button>" +
    "<div class='btn-group-vertical w-100' role='group' id='dd" +
    index1 +
    "'></div>";
  return str;
}
function makeLeftBarDDMenu(index1) {
  for (let i = 0; i < leftBarArrAll.length; i++) {
    if (i != index1) document.getElementById("dd" + i).innerHTML = "";
    else {
      let str = leftBarArr[index1]
        .map(function (ele, index2) {
          return (
            "<button class='btn btn-secondary w-100 h-100'type='button' onclick='showOptionsLeftDD(" +
            index1 +
            "," +
            index2 +
            ")' >" +
        ...
        .join("");
      document.getElementById("dd" + i).innerHTML = str;
    }
  }
}
// function makeLeftDD(txt, index1, open, options, actionsData) {
...
//         .join("") +
//       "</div>";
//   if (actionsData) str += actionsData.strMenu;
//   return str;
// }
function showOptionsLeftDD(index1, index2) {
  if (index2 === -1) {
    leftBarOpenStatus[index1] = !leftBarOpenStatus[index1];
    makeLeftBarDDMenu(index1);
  } else {
    let strMenu = dd1Actions(index2);
    console.log(strMenu);
    makeLeftBar({ dd: index1, option: index2, strMenu: strMenu });
  }
}
function dd1Actions(index) {
  let dd1ActionsFns = [
    defaultBoardUI1,
    changeBoardColorUI,
...
function colorPickerFns() {
  const colorPicker1 = document.getElementById("colorPicker1");
  const colorPicker2 = document.getElementById("colorPicker2");
  if (colorPicker1) {
    console.log("colorPickerFns added");
    colorPicker1.addEventListener("input", () => colorChanged(1));
    colorPicker2.addEventListener("input", () => colorChanged(2));
    colorPicker1.addEventListener("focus", updateBoxShadow);
    colorPicker1.addEventListener("blur", resetBoxShadow);
...
//   });
//   leftStr =
//     "<div class = 'containerLeft'><div class='btn-group-vertical w-100' role='group'><button class = 'p-3 btn btn-light btn-block-red w-100 h-100' onclick = 'undoMove()'>Undo Move</button><select class='p-3 btn btn-light left-bar-block w-100 h-100' id = 'dd1' onChange=dd1Actions()><option selected disabled>" +
//     leftBarArrAll[0] +
//     "</option>" +
//     arr1.join("") +
...
function defaultFunctionSettings() {
  ...
  isLoadingPGNPawnPromotionBool = false;
  lastMoveJSON = {
    prevrow: -1,
    prevcol: -1,
    newrow: -1,
  ...
  if (prevrow === -1 || prevcol === -1) {
    ...
    if (obj) {
      ...
      if (
        ...
        if (!isLoadingPGNPawnPromotionBool) {
...
function sumPoints(color) {
  return boardArr.reduce(function (ans, ele) {
    return (
      ans +
      ele.reduce(function (ans1, ele1) {
        return ele1.color === color ? ans1 + ele1.points : ans1;
      }, 0)
    );
  }, 0);
}
function pointDifference() {
  let pointsStr = "<br>";
  let whiteSum = sumPoints("white");
  let blackSum = sumPoints("black");
  if (whiteSum > blackSum) {
    pointsStr =
      "<p class = 'text-white'>+" + (whiteSum - blackSum) + "(white)</p>";
  } else if (blackSum > whiteSum) {
    pointsStr =
      "<p class = 'text-black'>+" + (blackSum - whiteSum) + "(black)</p>";
  } else {
    pointsStr = "";
  }
  return pointsStr;
...
function importGame() {
  ...
  showPGN();
  isLoadingPGNPawnPromotionBool = true;
  decodePGN();
  isLoadingPGNPawnPromotionBool = false;
  makeBoard();
}
      ...
      for (let j = 0; j < moveStrdecodePGN.length; j++) {
        ...
        if (moveStrdecodePGN[j] === "=") {
          ...
          if (moveStrdecodePGN[j + 1] === "N") {
            ...
            lastMovePGN.promotedtoPiecePoints = 3;
          } else if (moveStrdecodePGN[j + 1] === "R") {
            lastMovePGN.promotedtoPiece = "rook";
            lastMovePGN.promotedtoPiecePoints = 5;
            ...
            lastMovePGN.promotedtoPiecePoints = 9;
          }
        }
      }
      if (
...
function makeBoardViaPGN(lastMovePGN) {
  ...
  if (!lastMovePGN.castleBool) {
    ...
    if (lastMovePGN.promotionBool) {
      boardArr[lastMovePGN.newrow][lastMovePGN.newcol] = {
...
function showPGN() {
  ...
  menuStr +=
    "<button class = 'p-3 btn btn-light btn-timer w-100 h-100' id='copyPGN'><i class='fa-solid fa-copy'></i> Copy PGN</button>";
...
function importPGNUI() {
  let menuStr =
    "<span class = 'text-area-block w-100 justify-content-center'>Enter PGN : </span><div class='input-group'><textarea class='input-group-text text-area-block w-100' id='moveHistory'></textarea></div><button class = 'p-3 btn btn-light btn-timer w-100 h-100' onclick = 'importGame()'>Load Game</button>";
  document.getElementById("dd3menu").innerHTML = menuStr;
...
function switchNavTab_LoadGame() {
  ...
  function setActiveTab(tabName) {
    ...
    // Find the nav-link with the href matching the specified tabName and add 'active' class
    const targetLink = document.querySelector(
      `.navbar-nav .nav-link[href="#${tabName}"]`
    );
    if (targetLink) {
...
/*
PGNs
1.Nf3 Nc6 2.Nc3 Nf6 3.Ne4 Ne5 4.Neg5 Nfg4
1.e4 e5 2.d4 d5 3.Nh3 Na6 4.Na3 Nh6 5.dxe5 dxe4 6.Qxd8+ Kxd8 7.Bb5 c5 8.Bxa6 bxa6 9.Bxh6 gxh6 10.Nc4 Bxh3 11.gxh3 e3 12.Nxe3 Rg8 13.Rd1+ Ke8 14.Rd5 
1.e4 d5 2.exd5 Bg4 3.Nf3 Nc6 4.Bb5 Qd6 5.O-O O-O-O
1.e4 d5 2.exd5 Bg4 3.Nf3 Nc6 4.Bb5 Qd6 5.O-O O-O-O 6.dxc6 Kb8 7.cxb7 Rc8 8.bxc8=Q+
1.e4 d5 2.c4 f5 3.g4 b5 4.a4 h5 5.exf5 dxc4 
1.e4 d5 2.Bd3 Be6 3.Nc3 Nc6 4.Nh3 Qd7 5.Qf3 Nf6 6.Bb5 g6 7.b3 Bg7 8.Bb2 Rc8 9.Rf1 Nh5 10.a3 
1.e4 d5 2.exd5 Bg4 3.Nf3 Nc6 4.Bb5 Qd6 5.O-O O-O-O 6.dxc6 Kb8 7.cxb7 Rc8
*/
...
function playMoveSimulator(newMoveSimulator) {
  ...
  console.log("Played move", movesSimulated, newMoveSimulator);
  movesSimulated.do++;
}
function playToDepth(boardPosition, depth, moveColor) {
  console.log("playToDepth", depth, moveColor);
